...
for n in $(seq 1 30); do [ -f "$ROOT/web/$COURSE/scripts/lesson-$(printf %02d $n).json" ] || continue; echo "--- $COURSE L$n $(date +%H:%M)"; python3 synth.py --course "$COURSE" --lesson "$n" | tail -1; python3 assemble.py --course "$COURSE" --lesson "$n" | head -1; done
